...
| `ua_encoding_check.py` | **present** — written 16 Sep 2026, after the same character was found written two ways three times in two days. Reports the split; `--term` expands a search into both forms; `--selftest` |
| `ua_lastmod_check.py` | **present** — written 16 Sep 2026, after the `lastmod` rule lapsed on the one commit where the job was about wording. Compares every entry to `git log`; `--selftest` |
...
**It happened again on 16 September 2026, on `ua_encoding_check.py` itself.** The check exists because the same character gets written two ways. Its `--term` expander, handed a term already in entity form, re-escaped the ampersand of that entity: `&euro;880,000` came back as `&amp;euro;880,000`. **A check written to catch a mixed estate was mixing the two directions inside itself.**

**The fixtures caught it before it was committed, and the reason they could is the point.** They were built from the two real failures, `H&M` and `&euro;880,000`, rather than from what the author thought the failure looked like. **A fixture drawn from the defect that actually happened survives its author's model of it; one drawn from the model does not.**
...
### `lastmod` AND RESUBMISSION ARE TWO DIFFERENT TRIGGERS. DO NOT CONFLATE THEM
**Resubmission is about the entry list.** It is needed only when a URL is added or removed, exactly as above.

**`lastmod` is about content, and it moves whenever the page does.** Nothing said so until 30 August 2026, and the omission cost a sweep: **43 of 45 entries carried a `lastmod` older than the page's last change**, some by seven weeks. `insights/what-eaa-compliance-actually-requires.html` said 8 July on a page corrected that morning.

**A stale `lastmod` works against a submission.** It tells a crawler the page has not changed, on the same day someone asks it to recrawl.

### DERIVE IT FROM THE REPO. NEVER TYPE IT
```bash
git log -1 --date=short --pretty=%ad -- <path>
```
```bash
python3 ua_lastmod_check.py .        # fails on any entry that disagrees with git log
```
**Run it after committing**, which is where it belongs: a page edited but not yet committed compares against its previous state, and the check says so rather than passing quietly.

**A hand-typed `lastmod` is a date asserted rather than observed**, which is the same class as an elapsed counter and a status stamp written from memory: it drifts silently and passes every check, because nothing compares it to anything. The sweep above was generated by walking every `<url>` block and reading the log for its path, and it should be regenerated the same way rather than edited by hand.

**Note the paths that need mapping:** `https://usableaccess.io/` is `index.html`, and nothing else in the file is bare.
...
```bash
python3 ua_encoding_check.py . --term "H&M"    # counts both forms, and says so
```
**The `--term` mode is the point.** It applies the rule at the moment of the search rather than asking anyone to remember it beforehand, which is what the rule failing against its own author ten minutes after being pushed showed was necessary. Run with no `--term` it prints the map of which characters are split and where.

**It reports rather than fails, deliberately.** Its first run found 57 files mixing a character, most of them em dashes, so "no page mixes forms" is not an achievable end state here and a gate set there would be tuned away. `--strict` restores the per-file failure for anyone normalising a page.
